...
-- No Render / Render Off (F4 Toggle)
local UserInputService = game:GetService("UserInputService")
local RunService = game:GetService("RunService")
local NoRenderState = true -- Activer par défaut

local function UpdateRenderState()
    -- Si NoRenderState est true, on désactive le rendu 3D (Enabled = false)
    -- Si NoRenderState est false, on active le rendu 3D (Enabled = true)
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == Enum.KeyCode.F4 then
        NoRenderState = not NoRenderState
        UpdateRenderState()
    end
end)
